...
function CaseStudies() {
  ...
  return (
    <section className="section case-studies-section" id="case-studies">
      <div className="shell">
        <div className="section-heading projects-heading reveal">
          <div><p className="eyebrow">03 · Applied Analysis</p><h2>Case studies grounded in measurable improvement.</h2></div>
          <p>Detailed examinations of manufacturing and engineering challenges using Lean Six Sigma, operational data, root-cause analysis, and practical improvement roadmaps.</p>
        </div>
        <div className="case-study-grid">
          {caseStudies.map(study => (
            <article className="project-card case-study-card reveal" key={study.title} style={{"--card-accent": accents[study.accent] || accents.cyan}}>
              <div className="project-top"><span className="project-icon">{icons[study.icon] || "◆"}</span><span className="project-version">{study.version}</span></div>
              <div className="project-content">
                <span className="project-status">{study.status}</span>
                <h3>{study.title}</h3>
                <p className="project-description">{study.description}</p>
                <div className="project-kpis">{study.kpis.map(k => <span key={k}>{k}</span>)}</div>
                <div className="project-tech">{study.tech.map(t => <span key={t}>{t}</span>)}</div>
              </div>
              <div className="project-actions">
                <a className="live-link" href={study.live} target="_blank" rel="noreferrer">{study.liveLabel} ↗</a>
                <a href={study.github} target="_blank" rel="noreferrer">GitHub</a>
              </div>
            </article>
          ))}
        </div>
...
function Capabilities() {
  ...
  return (
    <section className="section capabilities" id="capabilities">
      <div className="shell">
        <div className="section-heading reveal"><p className="eyebrow">04 · Capabilities</p><h2>A multidisciplinary toolkit for real-world improvement.</h2></div>
...
function Journey() {
  return (
    <section className="section journey" id="journey">
      <div className="shell">
        <div className="section-heading reveal"><p className="eyebrow">05 · Journey</p><h2>Education, service, and technical growth.</h2></div>
